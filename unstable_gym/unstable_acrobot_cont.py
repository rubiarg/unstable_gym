"""
Classic 2-link arm system implemented by Rich Sutton et al.
Copied from http://incompleteideas.net/sutton/book/code/acrobot.lisp
Implemented based on OpenAI Gym Acrobot https://github.com/rlpy/rlpy/blob/master/rlpy/Domains/Acrobot.py
"""
...
class UnstableAcrobotContEnv(core.Env):
    ...
    def step(self, a):
        ...
        torque = max(min(a, self.TORQUE_MAX), self.TORQUE_MIN)
    ...
    def _dsdt(self, s_augmented):
        ...
        if self.book_or_nips == "nips":
            ...
            # book
            ddtheta2 = (
                a + d2 / d1 * phi1 - m2 * l1 * lc2 *
                dtheta1**2 * sin(theta2) - phi2
            ) / (m2 * lc2**2 + I2 - d2**2 / d1)
    ...
    def render(self, mode="human"):
        ...
        # draw a gray line for episodic setting
        # pygame.draw.line(
        #     self.surf,
        ...
        for ((x, y), th, llen) in zip(xys, thetas, link_lengths):
            ...
            gfxdraw.filled_polygon(
                self.surf, transformed_coords, (0, 204, 204))

            gfxdraw.aacircle(self.surf, int(x), int(
                y), int(0.1 * scale), (204, 204, 0))
